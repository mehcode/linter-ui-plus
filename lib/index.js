...
function activate () {
  ...
  atom.commands.add('atom-workspace', {
    'linter-ui-plus:toggle-panel': togglePanel,
...
// TODO: Ask why this doesn't have the word Linter in it
function provideUI () {
  return {
    name: 'linter-ui-plus',
...
async function didSelect (sel: ?Selected, options: ?SelectOptions) {
  ...
  if (panelContainer != null) {
    panelContainer.updateSelected(selected, options)

    // Ensure that the panel is focused
    if (!options || !options.activate) {
      panelContainer.element.focus()
    }
  }
}

module.exports = { activate, deactivate, provideUI }
